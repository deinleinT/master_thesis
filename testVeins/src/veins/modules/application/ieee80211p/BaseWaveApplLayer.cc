...
void BaseWaveApplLayer::initialize(int stage) {
    ...
    if (stage == 0) {
        ...
        if (beaconWAVE) {
            ...
            offSet = dblrand() * (defaultBeaconInterval.dbl() / 2);
...
void BaseWaveApplLayer::performCAMGenerating() {
    ...
    if (strcmp(getParentModule()->getFullName(), "node[0]") == 0) {
        if (lastSentCAM != nullptr && lastSentCAMMove != nullptr) {
            ...
            std::cout << "***CheckCamParamsInterval simTime " << simTime()
                    << " timeElapsed " << timeElapsed << " T_GenCam "
                    << T_GenCam << " T_GenCamDCC " << T_GenCam_DCC << std::endl;
        }
    }
    //Condition one
    if (lastSentCAM != nullptr && lastSentCAMMove != nullptr) {
            ...
            if (difSpeed > 0.5 || difPosition > 4 || difDirection > 4) {
                ...
                if (strcmp(getParentModule()->getFullName(), "node[0]") == 0) {
                    if (lastSentCAM != nullptr && lastSentCAMMove != nullptr) {
                        ...
                        std::cout << "CheckCamParams CONDITION ONE simTime "
                                << simTime() << " directionDifference "
                                << difDirection << " speedDifference "
                                << difSpeed << " positionDistance "
                                << difPosition << " timeElapsed " << timeElapsed
                                << " T_GenCam " << T_GenCam << std::endl;
                ...
                if (counterNumGenCam == numGenCam) {
                    ...
                    if (strcmp(getParentModule()->getFullName(), "node[0]")
                            == 0) {
                        std::cerr
                                << "CheckCamParams CONDITION TWO COUNTER MAX simTime "
                                << simTime() << " timeElapsed " << timeElapsed
                                << " T_GenCam " << T_GenCam << std::endl;
                    }
                }
                ...
                if (strcmp(getParentModule()->getFullName(), "node[0]") == 0) {
                    std::cout << "CheckCamParams CONDITION TWO simTime "
                            << simTime() << " timeElapsed " << timeElapsed
                            << " T_GenCam " << T_GenCam << std::endl;
                }
...
//        scheduleAt(simTime() + T_GenCam, msg);
    ...
    case SEND_BEACON_BSM: {
        ...
        calculateRadiatedPowerAndSetInMacLayer();
...
WaveShortMessage* BaseWaveApplLayer::prepareWSM(std::string name,
    ...
    if (beaconWAVE) {
        ...
        //see 102 687, V1.2
        temp->setCBR_R_0_Hop(
                macLayer->getChannelLoadListenerETSI()->getCbrValues().CBR_L_0_Hop);
        ...
        temp->setCBR_R_1_Hop(
                macLayer->getChannelLoadListenerETSI()->getCbrValues().CBR_L_1_Hop);
...
void BaseWaveApplLayer::performCBPMeasureInterval() {
    ...
    channelBusyPercentageWAVE =
            macLayer->getChannelBusyWaveListener()->calculateChannelBusyPercentage(
                    ...
                    totalBusyTime, testCaseNumber);

    channelBusyPercentageVec.record(channelBusyPercentageWAVE);
//
}

void BaseWaveApplLayer::calculateRadiatedPowerAndSetInMacLayer() {

    if (testCaseNumber > 7 && testCaseNumber < 8)
        performCalculatingRadiatedPowerTest();
...
//RADIATED POWER 6.3.8.7, SAE J2945, Eq. 8 - 11
    ...
    if (testCaseNumber > 7)
        assertCalculatingRadiatedPower();
...
//timPositionChange saves the time, at which the position changed the last time
    ...
    if (testCaseNumber > 4 && testCaseNumber < 5)
        assertTrackingErrorTests();
...
//calculate transmission probability
    ...
    //tests
    if (testCaseNumber > 5)
        prepareTransmissionProbabilityTests();
    ...
    //assert tests
    if (testCaseNumber > 5 && testCaseNumber < 6)
        assertTransmissionProbabilityTest();
...
void BaseWaveApplLayer::calculateMaximumInterTransmitTime() {
    ...
    if (testCaseNumber > 6.1)
        assertMaxInterTransmitTimeTest();
...
void BaseWaveApplLayer::performScheduleDecision() {
...
//do nothing
//        std::cerr << "Car " << getParentModule()->getFullName()
//                << "Schedule Transmission DO NOTHING " << std::endl;

    }
}

void BaseWaveApplLayer::assertPERTestCases() {
//for test purposes
    if (testCaseNumber == 3.1) {
//ASSERT
        ASSERT(channelQualityIndicator == 0.0);
        ASSERT(vehicleDensityInRange == 0.0);
    } else if (testCaseNumber == 3.2) {
        ASSERT(channelQualityIndicator == 0.0);
        ASSERT(vehicleDensityInRange == 0);
    } else if (testCaseNumber == 3.3 || testCaseNumber == 3.4) {
        ASSERT(channelQualityIndicator == 0.3);
        ASSERT(vehicleDensityInRange == 1);
    } else if (testCaseNumber == 3.5) {
        ASSERT(channelQualityIndicator == 0.0);
        ASSERT(vehicleDensityInRange == 0);
    } else if (testCaseNumber == 3.6) {
        ASSERT(channelQualityIndicator == 0.3);
        ASSERT(vehicleDensityInRange == 2);
    }
}

void BaseWaveApplLayer::assertTrackingErrorTests() {
    if (testCaseNumber == 4.1) {
        ASSERT(trackingError == 0);
    } else if (testCaseNumber == 4.2) {
        ASSERT(trackingError == 0);
    } else if (testCaseNumber == 4.3) {
        ASSERT(trackingError == 1);
    } else if (testCaseNumber == 4.4) {
        ASSERT(trackingError >= 0.95 && trackingError <= 1.01);
    }
}

void BaseWaveApplLayer::prepareTransmissionProbabilityTests() {
    //calculate transmission probability
    //
    if (testCaseNumber == 5.1) {
        trackingError = 0.19;
...
void BaseWaveApplLayer::assertTransmissionProbabilityTest() {
    if (testCaseNumber == 5.1) {
        ASSERT(transmissionProbability == 0);
    } else if (testCaseNumber == 5.2) {
        ASSERT(transmissionProbability == 0);
    } else if (testCaseNumber == 5.3) {
        ASSERT(transmissionProbability >= 0.99 && transmissionProbability <= 1);
    } else if (testCaseNumber == 5.4) {
        ASSERT(transmissionProbability == 1);
    } else if (testCaseNumber == 5.5) {
        ASSERT(transmissionProbability == 1);
    }
}

void BaseWaveApplLayer::assertMaxInterTransmitTimeTest() {
    if (testCaseNumber == 6.2) {
        ASSERT(maximumInterTransmitTime = 0.1);
    } else if (testCaseNumber == 6.3) {
        ASSERT(maximumInterTransmitTime = 0.104);
    } else if (testCaseNumber == 6.4) {
        ASSERT(maximumInterTransmitTime = 0.596);
    } else if (testCaseNumber == 6.5) {
        ASSERT(maximumInterTransmitTime = 0.6);
    }
}

void BaseWaveApplLayer::performCalculatingRadiatedPowerTest() {
    if (testCaseNumber == 7.1) {
        txDecision_Dynamics = true;
        ...
        txDecision_Critical_Event = false;
        txDecision_Dynamics = false;
        channelBusyPercentageWAVE = vMinChanUtil + 1;
        ...
        txDecision_Critical_Event = false;
        txDecision_Dynamics = false;
        channelBusyPercentageWAVE = vMaxChanUtil + 1;
    } else if (testCaseNumber == 7.6) {
        txDecision_Critical_Event = false;
        txDecision_Dynamics = false;
        channelBusyPercentageWAVE = 100;
    }
}

void BaseWaveApplLayer::assertCalculatingRadiatedPower() {
    if (testCaseNumber == 7.1) {
        ASSERT(rp == vRPMax);
    } else if (testCaseNumber == 7.2) {
        ASSERT(fCBP == vRPMax);
    } else if (testCaseNumber == 7.3) {
        ASSERT(fCBP >= 19.6 && fCBP <= 19.7);
    } else if (testCaseNumber == 7.4 || testCaseNumber == 7.5
            || testCaseNumber == 7.6) {
        ASSERT(fCBP == vRPMin);
    }
}
...
//CBP
    ...
    if (sendData) {
        recordScalar("Received Data", counterReceivedData);
    }
