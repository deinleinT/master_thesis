...
ChannelLoadListenerEtsi::ChannelLoadListenerEtsi(
        omnetpp::cSimpleModule * source, double * NDL_refPacketInterval, bool useGateKeeping) :
...
double ChannelLoadListenerEtsi::calculateChannelLoad(simtime_t period,
    ...
    if (testCaseNumber == 1.6) {
        ...
        CBP = TestCaseHandler::getTestValueETSI(testCaseNumber);
    }
    ...
    //TODO
    calculateCBR_G();
    //reset cbrValues, cause Lifetime of them only valid in a Period of 100ms
    //TODO see TS 102 687 V1.2
    calculateTargetRate();
...
CBRValues & ChannelLoadListenerEtsi::setCbrValues() {
    return cbrValues;
}
...
void ChannelLoadListenerEtsi::calculateTargetRate() {
    ...
    ASSERT(currentCBR >= 0 && lastCbrValues.CBR_G >= 0);

    double CBR_DCC = fmax(currentCBR, lastCbrValues.CBR_G);
    double r = CBR_DCC * 2000.0;
    double rg = 0.8 * 2000; // is target_rate, constant for all vehicles, how successful is reception of packets
...
double ChannelLoadListenerEtsi::getMinChannelLoad(simtime_t interval,
    ...
    std::vector<double> cbpVector;

    for (auto value : addedTimes) {
        if (value.first > intervalstart && value.first <= simTime()) {
            cbpVector.push_back(value.second / period);
        }
    }

    return *std::min_element(cbpVector.begin(), cbpVector.end());
}
...
double ChannelLoadListenerEtsi::getMaxChannelLoad(simtime_t interval,
    ...
    return *std::max_element(cbpVector.begin(), cbpVector.end());
}
