...
template double BasePhyLayer::readPar<double>(const char* parName,
        const double);
template simtime_t BasePhyLayer::readPar<simtime_t>(const char* parName,
        const simtime_t);
...
void BasePhyLayer::initialize(int stage) {
    ...
    if (stage == 0) {
        ...
        airframeSizeReceivingVec.setName("AiframeSize of received packets in byte");
        airframeSizeSendingVec.setName("AirframeSize of sent packets in byte");

        //read simple ned-parameters
        //	- initialize basic parameters
        if (par("useThermalNoise").boolValue()) {
            double thermalNoiseVal = FWMath::dBm2mW(
                    par("thermalNoise").doubleValue());
            thermalNoise = new ConstantSimpleConstMapping(
                    DimensionSet::timeDomain(), thermalNoiseVal);
        } else {
            thermalNoise = 0;
        }
        headerLength = par("headerLength").longValue();

        //changed Values for sensitivity
        if (getParentModule()->getParentModule()->par("beaconWAVE").boolValue()) {

            //see SAE J2945, Page 73
            sensitivity = par("sensitivityWAVE").doubleValue();
            std::cerr << "Sensitivity WAVE in Phy " << sensitivity << std::endl;

        } else if (getParentModule()->getParentModule()->par("beaconETSI").boolValue()) {

            sensitivity = par("NDL_defDccSensitivity").doubleValue();
            WATCH(sensitivity);
            std::cerr << "Sensitivity ETSI in Phy " << sensitivity << std::endl;
...
Radio* BasePhyLayer::initializeRadio() {
    ...
    radio->setSwitchTime(Radio::TX, Radio::SLEEP,
            (hasPar("timeTXToSleep") ?
                    par("timeTXToSleep") : par("timeRXToSleep")).doubleValue());
    //if no RX to sleep defined asume same time as TX to sleep
    radio->setSwitchTime(Radio::RX, Radio::SLEEP,
            (hasPar("timeRXToSleep") ?
                    par("timeRXToSleep") : par("timeTXToSleep")).doubleValue());

    return radio;
}

void BasePhyLayer::getParametersFromXML(cXMLElement* xmlData,
        ParameterMap& outputMap) {
    ...
    for (cXMLElementList::const_iterator it = parameters.begin();
        ...
        if (name == 0 || type == 0 || value == 0)
            throw cRuntimeError(
                    "Invalid parameter, could not find name, type or value");
...
void BasePhyLayer::initializeDecider(cXMLElement* xmlConfig) {
    ...
    if (deciderList.empty()) {
        throw cRuntimeError(
                "No decider configuration found in configuration file.");
    }

    if (deciderList.size() > 1) {
        throw cRuntimeError(
                "More than one decider configuration found in configuration file.");
    }
    ...
    if (name == 0) {
        throw cRuntimeError(
                "Could not read type of decider from configuration file.");
    }
    ...
    if (decider == 0) {
        throw cRuntimeError("Could not find a decider with the name \"%s\".",
                name);
    }
...
void BasePhyLayer::initializeAnalogueModels(cXMLElement* xmlConfig) {
    ...
    if (newAnalogueModel == 0) {
        throw cRuntimeError(
                "Could not find an analogue model with the name \"%s\".",
                s.c_str());
    } else {
    ...
    cXMLElementList analogueModelList = xmlConfig->getElementsByTagName(
            "AnalogueModel");

    if (analogueModelList.empty()) {
        throw cRuntimeError(
                "No analogue models configuration found in configuration file.");
    }
    ...
    for (cXMLElementList::const_iterator it = analogueModelList.begin();
        ...
        AnalogueModel* newAnalogueModel = getAnalogueModelFromName(name,
                params);
...
void BasePhyLayer::handleAirFrameReceiving(AirFrame* frame) {
    ...
    coreEV << "Handed AirFrame with ID " << frame->getId()
...
void BasePhyLayer::handleUpperMessage(cMessage* msg) {
    ...
    if (radio->getCurrentState() != Radio::TX) {
        ...
        msg = 0;
        throw cRuntimeError(
                "Error: message for sending received, but radio not in state TX");
    }
    ...
    if (txOverTimer->isScheduled()) {
        ...
        msg = 0;
        throw cRuntimeError(
                "Error: message for sending received, but radio already sending");
    }
    ...
    //TODO
//    std::cerr
//            << "BaseLayer handleUpperMsg SENDING an Airframe frameLength in byte "
//            << frame->getByteLength() << std::endl;
    airframeSizeSendingVec.record(frame->getByteLength());
...
void BasePhyLayer::handleChannelSenseRequest(cMessage* msg) {
    ...
    if (nextHandleTime >= simTime()) { //schedule request for next handling
        ...
        throw cRuntimeError(
                "Next handle time of ChannelSenseRequest returned by the Decider is smaller then current simulation time: %.2f",
                SIMTIME_DBL(nextHandleTime));
    }
...
void BasePhyLayer::handleUpperControlMessage(cMessage* msg) {
    ...
    default:
        throw cRuntimeError(
                "Received unknown control message from upper layer!");
    }
}
...
void BasePhyLayer::filterSignal(AirFrame *frame) {
    ...
    ChannelAccess * const receiverModule =
            dynamic_cast<ChannelAccess * const >(frame->getArrivalModule());
    ...
    ChannelMobilityPtrType sendersMobility =
            senderModule ? senderModule->getMobilityModule() : NULL;
...
BasePhyLayer::~BasePhyLayer() {
    ...
    for (AirFrameVector::iterator it = channel.begin(); it != channel.end();
            ++it) {
        cancelAndDelete(*it);
    }
...
void BasePhyLayer::finishRadioSwitching() {
    ...
    sendControlMsgToMac(
            new cMessage("Radio switching over", RADIO_SWITCHING_OVER));
}
...
void BasePhyLayer::setCurrentRadioChannel(int newRadioChannel) {
    if (txOverTimer && txOverTimer->isScheduled()) {
        EV_WARN
                       << "Switched channel while sending an AirFrame. The effects this would have on the transmission are not simulated by the BasePhyLayer!";
    }
...
void BasePhyLayer::getChannelInfo(simtime_t_cref from, simtime_t_cref to,
        AirFrameVector& out) {
...
void BasePhyLayer::sendUp(AirFrame* frame, DeciderResult* result) {
    ...
    coreEV << "Decapsulating MacPacket from Airframe with ID " << frame->getId()
                  << " and sending it up to MAC." << endl;

    //TODO
//    std::cerr << "BasePhyLayer receiving AirFrame sendUp framelengt in byte "
//            << frame->getByteLength() << std::endl;

    airframeSizeReceivingVec.record(frame->getByteLength());
...
void BasePhyLayer::cancelScheduledMessage(cMessage* msg) {
    if (msg->isScheduled()) {
        ...
        EV
                  << "Warning: Decider wanted to cancel a scheduled message but message"
...
void BasePhyLayer::recordScalar(const char *name, double value,
        const char *unit) {
