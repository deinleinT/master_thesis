...
const simsignalwrap_t TraCIMobility::parkingStateChangedSignal =
...
void TraCIMobility::finish() {
    ...
    if (simTime() - statistics.startTime > 6) {
...
void TraCIMobility::nextPosition(const Coord& position, std::string road_id,
        double speed, double angle,
        TraCIScenarioManager::VehicleSignal signals) {
    if (debug)
        EV << "nextPosition " << position.x << " " << position.y << " "
                  << road_id << " " << speed << " " << angle << std::endl;
    isPreInitialized = false;
    this->roadPosition = position;
    this->road_id = road_id;
    this->speed = speed;
...
double TraCIMobility::calculateCO2emission(double v, double a) const {
    ...
    if (P_tract <= 0)
        return alpha1;
    return alpha + beta * v * 3.6 + delta * v * v * v * (3.6 * 3.6 * 3.6)
            + zeta * a * v;
}
...
Coord TraCIMobility::calculateAntennaPosition(const Coord& vehiclePos) const {
    ...
    if (antennaPositionOffset >= 0.001) {
        ...
        corPos = Coord(vehiclePos.x - antennaPositionOffset * cos(angle),
                vehiclePos.y + antennaPositionOffset * sin(angle),
                vehiclePos.z);
    } else {
