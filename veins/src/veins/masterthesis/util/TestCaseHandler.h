...
public:
    static std::string getDescription(double testCasenumber) {
        ...
        if (testCasenumber == 0.0) {
            ...
            return " TestCase WAVE 4.1, run Test within TrackingErrorCalculator and confirm Tests with ASSERT (deltaTimeHVLocalEstimate = 0.06 --> trackingError is 0) ";

        } else if (testCasenumber == 4.2) {

            return " TestCase WAVE 4.2, run Test within TrackingErrorCalculator and confirm Tests with ASSERT (deltaTimeHVRemoteEstimate = 0.06 --> trackingError is 0) ";
    ...
    static double getTestValueETSI(double testCaseNumber) {
        ...
        if (testCaseNumber == 1.1) {
            ...
            if (omnetpp::simTime() >= 1 && omnetpp::simTime() <= 1.1) {
                return 0.20;
            } else if (omnetpp::simTime() >= 6 && omnetpp::simTime() <= 11.1) {
                return 0.8;
            } else {
                return 0.1;
            }
